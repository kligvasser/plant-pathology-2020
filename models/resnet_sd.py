...
class ResNet(nn.Module):
    def __init__(self, num_classes=4, depth=18, two_stage=False):
        ...
        if depth == 18:
            self.backbone = resnet18_StoDepth_lineardecay(pretrained=True)
        elif depth == 34:
            self.backbone = resnet34_StoDepth_lineardecay(pretrained=True)
        elif depth == 50:
            self.backbone = resnet50_StoDepth_lineardecay(pretrained=True)
        elif depth == 101:
            self.backbone = resnet101_StoDepth_lineardecay(pretrained=True)
        else:
            self.backbone = resnet152_StoDepth_lineardecay(pretrained=True)
