...
def get_params():
    ...
    exp_scheduler = rand_flag('--exp-scheduler', 0.9)
    auc = random.uniform(0.85, 1.0)
    acc = 1 - auc

    return {'crop_size': crop_size, 'crop_scale': crop_scale, 'lr': lr, 'step_size': step_size, 'epochs': epochs, 'seed': seed, 'tta': tta, 'auc': auc, 'acc': acc, 'exp_scheduler': exp_scheduler}

def set_cmd(args, params):
    cmd = 'python3 main.py --model efficientnet_ns --root {} --crop-size {} --crop-scale {} --train-cross-validation --lr {:.6f} --step-size {} --gamma 0.2 --epochs {} --batch-size 16 --device-ids 0 1 2 3 --seed {} {} {}'.format(args.root, params['crop_size'], params['crop_scale'], params['lr'], params['step_size'], params['epochs'], params['seed'], params['tta'], params['exp_scheduler'])
    return cmd
